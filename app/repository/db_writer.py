...
def _mysql_literal(value: Any) -> str:
    """
    Convert a Python value into a safe MySQL literal for a standalone SQL script.
    """
    if value is None:
        return "NULL"
...
def generate_upsert_sql_script(
    mappings: List[Dict[str, Any]],
    master_id: int,
    client_name: str,
    process_name: str,
    ...
    """
    Create a standalone SQL script that inserts/updates mapping rows.
    Intended for downloading from the pipeline ZIP and running manually in MySQL.
    """
    table = _get_table_name(settings)
    ...
    for m in mappings:
        ...
        if not (m.get("partner_field") or "").strip():
            skipped += 1
            continue
        rows.append(_build_row(m, master_id, client_name, process_name))
    ...
    header = [
        f"-- Auto-generated by lp-mapping-ai",
    ...
    if not rows:
        return "\n".join(header + ["-- No rows to write.", "COMMIT;"])

    values_sql = []
    for r in rows:
        values_sql.append(
            "(" + ", ".join(_mysql_literal(r.get(c)) for c in columns) + ")"
        )
