...
async def full_pipeline(
    ...
    include_sql_insert_script: bool = Form(
        True,
        description="(Deprecated: script is always included) If true, include a runnable SQL insert/upsert script in the output ZIP",
    ),
    ...
    try:
        ...
        # ── Generate DB upsert SQL script (also saved to pipeline_output) ───────
        sql_text = generate_upsert_sql_script(
            mappings=all_mappings,
            master_id=master_id,
            client_name=client_name,
            process_name=process_name,
        ...
        sql_filename = f"db_upsert_{safe_client_name}_{safe_process_name}_master_{master_id}.sql"
        sql_path = out_dir / sql_filename
        try:
            sql_path.write_text(sql_text, encoding="utf-8")
        except Exception as exc:
            logger.error(f"Could not write SQL script to {sql_path}: {exc}")
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            # Always include the runnable DB upsert script in the ZIP artifact.
            zf.writestr(sql_filename, sql_text)
